...
Forwarding `X-Forwarded-For` matters: the public API is rate-limited per client IP (60 requests/minute), and without the header every visitor looks like the proxy. To serve a status page on a dedicated hostname, see [Status pages](/docs/status-pages#serving-a-page-on-its-own-hostname).
...
Raw heartbeats don't accumulate forever. A background job (runs daily, plus a sweep shortly after boot) aggregates heartbeats older than the retention window into per-day statistics — uptime percentage and average response time — and then deletes the raw rows. Uptime history effectively lives forever as daily rollups; only the per-check granularity expires.
